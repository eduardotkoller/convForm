  ...
  if (this.current.hasNext()) {
    ...
    if (
      ...
      if (
        ...
        this.answers[this.current.input.fork].value != this.current.input.case
      ) {
        return this.next();
      }
  ...
  setTimeout(
    function () {
      ...
      if (
        ...
        this.current.input.noAnswer === true
      ) {
        if (this.next()) {
          setTimeout(
            function () {
...
ConvState.prototype.printAnswers = function (answers, multiple) {
  ...
  if (multiple) {
    for (var i in answers) {
      if (answers.hasOwnProperty(i)) {
        var option = $('<div class="option">' + answers[i].text + "</div>")
          ...
          .click(
            function (event) {
              if (!Array.isArray(this.current.input.selected))
                this.current.input.selected = [];
              var indexOf = this.current.input.selected.indexOf(
                $(event.target).data("answer").value
              );
              if (indexOf == -1) {
    ...
    var optionsContainer = this.wrapper.find("div.options");
    var rowContainer; // Remove the initial row container creation
    for (var i in answers) {
      if (answers.hasOwnProperty(i)) {
        // Create a new row container after every third option
        if (i % 3 === 0) {
        ...
        var option = $('<div class="option">' + answers[i].text + "</div>")
          ...
          .click(
            function (event) {
              this.current.input.selected = $(event.target).data(
                "answer"
              ).value;
              this.wrapper
                .find(this.parameters.inputIdHashTagName)
              ...
              this.wrapper.find(this.parameters.inputIdHashTagName).val("");
              this.answerWith(
                $(event.target).data("answer").text,
                $(event.target).data("answer")
              );
              this.wrapper.find("div.options div.option").remove();
            }.bind(this)
          );

        rowContainer.append(option); // Append option to the row container

        // Append the row container to the options container after every third option
        if ((i + 1) % 3 === 0 || i == answers.length - 1) {
          optionsContainer.append(rowContainer);
        }
      }
    }
    $(window).trigger("dragreset");
  ...
  if (!opened) {
    ...
    $(this.wrapper)
      .find(".wrapper-messages")
      .css({ marginBottom: diff, maxHeight: originalHeight - diff });
  }
...
ConvState.prototype.answerWith = function (answerText, answerObject) {
  ...
  if (
    this.current.input.type == "select" &&
    this.parameters.selectInputStyle == "disable"
  ) {
    $(this.wrapper)
      .find("#" + this.parameters.inputIdName)
      .prop("disabled", false);
    ...
    this.current.input.type == "select" &&
    this.parameters.selectInputStyle == "hide"
  ) {
    if (!this.current.input.multiple) {
      $(this.wrapper)
        .find("#" + this.parameters.inputIdName)
  ...
  var diff = $(this.wrapper).find("div.options").height();
  var originalHeight = $(this.wrapper)
    .find(".wrapper-messages")
    .data("originalHeight");
  $(this.wrapper)
    .find(".wrapper-messages")
    .css({ marginBottom: diff, maxHeight: originalHeight });
  $(this.wrapper).find(this.parameters.inputIdHashTagName).focus();
...
(function ($) {
  $.fn.convForm = function (options) {
    ...
    var inputs = $(this)
      ...
      .map(function () {
        var input = {};
        if ($(this).attr("name")) input["name"] = $(this).attr("name");
        if ($(this).attr("data-no-answer")) input["noAnswer"] = true;
        if ($(this).attr("required")) input["required"] = true;
        if ($(this).attr("type")) input["type"] = $(this).attr("type");
    ...
    if (inputs.length) {
      ...
      $(wrapper).append(
        '<div class="wrapper-messages"><div class="spinLoader ' +
          parameters.loadSpinnerVisible +
      ...
      $(inputForm)
        .find(parameters.inputIdHashTagName)
        .keypress(function (e) {
          if (e.which == 13) {
            var input = $(this).val();
            e.preventDefault();
            if (
              state.current.input.type == "select" &&
              !state.current.input.multiple
            ) {
              if (state.current.input.required) {
                ...
                var results = state.current.input.answers.filter(function (el) {
                  return (
                    el.text.toLowerCase().indexOf(input.toLowerCase()) != -1
                  );
                });
                if (results.length) {
              ...
              if (input.trim() != "") {
                ...
                if (results.length) {
                  if (
                    state.current.input.selected.indexOf(results[0].value) == -1
                  ) {
      ...
      $(inputForm)
        ...
        .click(function (e) {
          ...
          if (
            ...
            if (
              state.current.input.required &&
              state.current.input.selected.length === 0
            ) {
              return false;
            } else {
      ...
      $(inputForm).submit(function (e) {
        ...
        if (state.current.input.type == "select") {
          if (!state.current.input.multiple) {
            state.answerWith(
              state.current.input.selected.text,
              state.current.input.selected
            );
          } else {
            state.answerWith(
              state.current.input.selected.join(", "),
              state.current.input.selected
            );
          }
        } else {
